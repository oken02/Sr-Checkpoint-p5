...
import "../../extra-credit/promise-map";

const readFile = function (filePath) {
  return new Promise((resolve, reject) => {
    fs.readFile(filePath, (err, contents) => {
      if (err) {
        console.log("ERORRRRRR" + filePath);
...
describe("▒▒▒ Extra Credit tests ▒▒▒", function () {
  describe("Promise.map", () => {
    ...
    describe("retornando una promesa del iterador", () => {
      ...
      beforeEach(() => {
        filePaths = ["1.txt", "2.txt", "3.txt"].map((filePath) =>
      ...
      it("rechaza la promesa del map si el iterador retorna una promesa que es rechazada", () => {
    ...
    describe("promesas en el arreglo de entrada", () => {
      ...
      beforeEach(() => {
        ...
        filePaths.push(
          readFile(path.join(__dirname, "./1.txt")).then((fileContents) =>
            fileContents.toString()
          )
        );
        ...
        filePaths.push(
          readFile(path.join(__dirname, "./3.txt")).then((fileContents) =>
            fileContents.toString()
          )
        );
      });

      it("llama al iterador de la función con el valor resuelto de la promesa", () => {
        const spy = sinon.spy();
        ...
        const mapPromise = Promise.map(filePaths, (file) => {
          ...
          if (file.search(".txt") !== -1) {
            return readFile(file).then((fileContents) =>
              fileContents.toString().toUpperCase()
            );
          } else {
        ...
        return mapPromise.then((files) => {
          ...
          expect(spy.calledWith(path.join(__dirname, "./2.txt"))).to.be.equal(
            true
          );
      ...
      it("rechaza la promesa de map si una promesa del arreglo de entrada es rechazada", () => {
